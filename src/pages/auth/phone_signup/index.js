...
const PhoneSignUp = (props) => {
  ...
  */
	...
	/* const onPhoneSignIn = () => props.history.push('phone_signin') */

	const onPhoneSignIn = () => props.history.push('/verification_phone');
					...
					<>
						<Grid container spacing={0} className={classes.signUpContentWrapper}>
							...
							<Grid item xs={12} container spacing={0} className={`signup ${signUpStatus.message && (!authStatus.success && 'input-error')}`}>
								<Grid item xs={12} className={classes.signUpMain}>
									<FormikProvider value={formik}>
										<form className={classes.form} noValidate autoComplete={false} onSubmit={onFormSubmit}>
							...
							<Grid item xs={12} container spacing={0} className={classes.signupMessage}>
								<Link onClick={onPhoneSignIn} className={classes.signUpLink} variant="body2" disabled={isDisabled}>
